...
=======
library(car)
>>>>>>> 31a74e4cddbef2b884562efcea7e144dcda59c6c
library(ggridges)
library(ggplot2)
library(viridis)
library(hrbrthemes)
...
=======
Trochę inny wykres od 'ggcorrplot(corr, method = "circle")', ale prowadzący do tych samych wniosków: dodatnia korelacja wzrostu z wagą i dodatnia korelacja wieku z wydatkami.
...
=======
```{r Dodatkowy wykres}
ggplot(df, aes(x = expenses, y = gender, fill = stat(x))) +
  geom_density_ridges_gradient() +
  labs(x = "Expenses",
       y = "Gender",
...
Hipoteza zerowa H0: $m = 165$
Hipoteza alternatywna H1: $m < 165$
>>>>>>> 31a74e4cddbef2b884562efcea7e144dcda59c6c
...
```
```{r}
wilcox.test(height, mu = 165, alternative = "less", correct = FALSE)
...
leftstud <- mean(age) - 1 / sqrt(n) *
  ...
  qt(1 - alpha / 2, (n - 1))
=======
rightstud <- mean(age) + 1 / sqrt(n - 1) *
  sd(age) *
  qt(1 - alpha / 2, (n - 1))
leftstud <- mean(age) - 1 / sqrt(n - 1) *
  sd(age) *
  qt(1 - alpha / 2, (n - 1))
